...
class TicketBotGUI:
    def __init__(self, root):sdsdfdsf
        self.root = root
            ...
            def handle_card_auth(driver):
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"⚠️ [初始Alert] {alert_text}")  # 新增：打印初始Alert内容
                    ...
                    if card_auth_detected:
                        ...
                        if card_input:
                            ...
                            if submit_button:
                                ...
                                # 快速檢測並處理提交後的確認彈窗
                                max_wait_time = 20   # 增加到10秒
                                check_interval = 0.5  # 保持100毫秒檢查一次
                                elapsed_time = 0
                                ...
                                while elapsed_time < max_wait_time:
                                    try:
                                        # 嘗試檢測彈窗
                                       # print(f"🔍 [尝试检测] 第 {int(elapsed_time/check_interval)+1} 次检测弹窗...")
                                        debug_log(f"嘗試檢測彈窗 - 第 {int(elapsed_time/check_interval)+1} 次", driver)
                                        
                                        alert = driver.switch_to.alert
                                        alert_text = alert.text
                                        update_status(f"檢測到確認彈窗: {alert_text}")
                                        debug_log(f"檢測到確認彈窗: {alert_text}", driver)
                                        print(f"🚨 [卡號認證弹窗] {alert_text}")
                                        
                                        # 檢查是否為錯誤信息
                                        error_keywords = [
                                            '不正確', '錯誤', '無效', '失敗', 
                                            'incorrect', 'error', 'invalid', 'fail',
                                            '請確認', '再試', '重新'
                                        ]
                                        ...
                                        if is_error:
                                            ...
                                            # 成功信息
                                            alert.accept()
                                            update_status("✅ 卡號認證成功，繼續下一步")
                                            debug_log("✅ 卡號認證成功，繼續下一步", driver)
                                            print(f"✅ [卡號成功] {alert_text}")
                                            time.sleep(0.5)
                                            return True
